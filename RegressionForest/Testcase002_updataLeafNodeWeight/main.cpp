...
#include "../RegressionForest_updataLeafNodeWeight/RegressionForestInterface.h"
#include "../RegressionForest_updataLeafNodeWeight/RegressionForestCommon.h"
#include "../RegressionForest_updataLeafNodeWeight/TrainingSet.h"
#include "../RegressionForest_updataLeafNodeWeight/TrainingSetConfig.h"
#include "../RegressionForest_updataLeafNodeWeight/TrainingSetCommon.h"
#include "../RegressionForest_updataLeafNodeWeight/Utility.h"
...
void devideTestToFile(std::vector<std::pair<int, float>>& vioBiasRateIndex, const std::vector<std::vector<float>>& vTestFeatureSet, const std::vector<float>& vTestResponseSet)
{
	...
	int StartDividePoint = CTrainingSetConfig::getInstance()->getAttribute<int>(hiveRegressionForest::KEY_WORDS::START_DIVIDE_POINT);
	int PrintDataSize = CTrainingSetConfig::getInstance()->getAttribute<int>(hiveRegressionForest::KEY_WORDS::NEW_FILE_DATA_SIZE);
	_ASSERTE(PrintDataSize <= vioBiasRateIndex.size() && (PrintDataSize + StartDividePoint) < vioBiasRateIndex.size());

	for (int i = StartDividePoint; i < PrintDataSize + StartDividePoint; ++i)
	{
...
void main()
{
	...
	try
	{
		...
		hiveRegressionForest::hivePredict(TestFeatureSet, TestResponseSet, PredictSet, ForestId);
