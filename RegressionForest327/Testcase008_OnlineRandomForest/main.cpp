...
#include "../RegressionForest_OnlineRandomForest/RegressionForestInterface.h"
#include "../RegressionForest_OnlineRandomForest/RegressionForestCommon.h"
#include "../RegressionForest_OnlineRandomForest/TrainingSet.h"
#include "../RegressionForest_OnlineRandomForest/TrainingSetConfig.h"
#include "../RegressionForest_OnlineRandomForest/TrainingSetCommon.h"
#include "../RegressionForest_OnlineRandomForest/Utility.h"
...
void main()
{
	...
	try
	{
		...
		std::vector<float> KnnPredictSet(TestFeatureSet.size(), 0.0f);
		std::vector<float> PredictSet(TestFeatureSet.size(), 0.0f);
		std::vector<float> LLPredictSet(TestFeatureSet.size(), 0.0f);
		std::vector<float> LLSet(TestFeatureSet.size(), 0.0f);
		std::vector<float> MPDissimilarity(TestFeatureSet.size(), 0.0f);
		std::vector<float> VarWiPerdict(TestFeatureSet.size(), 0.0f);
		clock_t PredictStart = clock();
		std::ofstream PredictInfo;
		PredictInfo.open("PredictInfo.csv", std::ios::app);
		for (auto Index = 0; Index < TestFeatureSet.size(); ++Index)
		{
			std::cout << "Predicting：" << Index << "/" << TestResponseSet.size() << std::endl;
			//PredictSet[Index] = hiveRegressionForest::hivePredict(TestFeatureSet[Index], ForestId, KnnPredictSet[Index], MPPredictSet[Index], LLPredictSet[Index], LLSet[Index],MPDissimilarity[Index], VarWiPerdict[Index], false);
			PredictSet[Index] = hiveRegressionForest::hivePredict(TestFeatureSet[Index], ForestId, KnnPredictSet[Index],  VarWiPerdict[Index], false);
 
			
			for (int i = 0;i < TestFeatureSet[Index].size();i++)
			{
				PredictInfo << TestFeatureSet[Index][i] << ",";
			}
			PredictInfo << PredictSet[Index] << "," << VarWiPerdict[Index] << std::endl;
		}
		...
		std::vector<float> KnnBiasSet(TestFeatureSet.size(), 0.0f);
		std::vector<float> KnnBiasRateSet(TestFeatureSet.size(), 0.0f);

		std::vector<float> VarWiBiasSet(TestFeatureSet.size(), 0.0f);
		std::vector<float> VarWiBiasRateSet(TestFeatureSet.size(), 0.0f);
		...
		for (int i = 0; i < TestFeatureSet.size(); ++i)
		{
			...
			KnnBiasSet[i] = std::abs(KnnPredictSet[i] - TestResponseSet[i]);
			KnnBiasRateSet[i] = KnnBiasSet[i] / TestResponseSet[i];
			VarWiBiasSet[i] = std::abs(VarWiPerdict[i] - TestResponseSet[i]);
			VarWiBiasRateSet[i] = VarWiBiasSet[i] / TestResponseSet[i];
		}
	...
	/*	for (int i = 0; i < TestFeatureSet.size(); ++i)
		...
		float KnnMSE = calMSE(KnnBiasSet);
		float KnnR2Score = calR2Score(TestResponseSet, KnnPredictSet);

		float VarWiMSE = calMSE(VarWiBiasSet);
		float VarWi2Score = calR2Score(TestResponseSet, VarWiPerdict);
		std::cout << "Final MSE: " << MSE << std::endl;
		std::cout << "Final R2Score: " << R2Score << std::endl; 
		std::cout << "Inter KNN MP MSE: " << KnnMSE << std::endl;
		std::cout << "Inter KNN MP R2Score: " << KnnR2Score << std::endl; 
		std::cout << "Var Weight MSE: " << VarWiMSE << std::endl;
		std::cout << "Var Weight R2Score: " << VarWi2Score << std::endl;
	/*	std::ofstream Coefficient;
		...
		StatisticalResultFile << "Inter KNN MP MSE: " << KnnMSE << std::endl;
		StatisticalResultFile << "Inter KNN MP R2Score: " << KnnR2Score << std::endl; 
		StatisticalResultFile << "Var Weight MSE: " << VarWiMSE << std::endl;
		StatisticalResultFile << "Var Weight R2Score: " << VarWi2Score << std::endl;

		StatisticalResultFile << "Predict Time: " << PredictEnd - PredictStart << std::endl;
