...
//****************************************************************************************************
//FUNCTION:
float CRegressionForest::predict(const std::vector<float>& vFeatures, unsigned int vNumOfUsingTrees, float& voMPPredictSet, float& voMPDissimilarity, bool vIsWeightedPrediction) const
{ 
	return __predictCertainResponse(vFeatures, vNumOfUsingTrees, vIsWeightedPrediction, voMPPredictSet, voMPDissimilarity);
}
...
//****************************************************************************************************
//FUNCTION:
float CRegressionForest::__predictCertainResponse(const std::vector<float>& vFeatures, unsigned int vNumOfUsingTrees, bool vIsWeightedPrediction, float& voMPPredictSet, float& voMPDissimilarity, unsigned int vResponseIndex) const
{
...
//#pragma omp parallel for-----------------计算中间节点的MP和LP--------------------------------
	...
	if (callMethod == 1)
	{
       /*#pragma omp parallel for*/
		...
		std::vector<std::tuple<std::vector<float>, float, float>> ResponseMPSet = pTrainingSet->calReCombineDataMP(LocatedNodeDataFeature, LocatedNodeDataResponse, vFeatures, SumPredict / vNumOfUsingTrees);
		...
		for (int i = 0; i < ResponseMPSet.size(); i++)
		{
			FeatureSet.push_back(std::get<0>(ResponseMPSet[i]));
		}

		//输出各位特征方差到文件
		/*std::ofstream PredictInfo;
		PredictInfo.open("PredictInfo.csv", std::ios::app);
		std::vector<float> FeatureStd(FeatureSet[0].size(), 0.0f);
		std::vector<float> FeatureMean(FeatureSet[0].size(), 0.0f);
		for (int i = 0; i < FeatureSet[0].size(); i++)
		{
			for (int k = 0; k < InstanceNumber; k++)
			{
				FeatureMean[i] += FeatureSet[k][i];
			}
			FeatureMean[i] /= InstanceNumber;
			for (int k = 0; k < InstanceNumber; k++)
			{
				FeatureStd[i] += (FeatureSet[k][i] - FeatureMean[i])*(FeatureSet[k][i] - FeatureMean[i]);
			}
			FeatureStd[i] = std::sqrt(FeatureStd[i] / InstanceNumber);
			PredictInfo << FeatureStd[i] << ",";
		}*/
		...
		else //直接计算均值
		{
			...
			for (int i = 0; i < InstanceNumber; ++i)
			{
				voMPPredictSet += std::get<1>(ResponseMPSet[i]);
				MP += std::get<2>(ResponseMPSet[i]);
			}
			voMPPredictSet = voMPPredictSet / InstanceNumber;
			MP = MP / InstanceNumber;
			//PredictInfo << MP << ",";
			//PredictInfo.close();
		}
	}
