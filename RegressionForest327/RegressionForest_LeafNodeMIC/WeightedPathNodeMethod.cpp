...
//****************************************************************************************************
//FUNCTION:
bool CWeightedPathNodeMethod::__isTotalInBoundBox(const std::vector<float>& vTestPoint, const std::vector<float>& vLow, const std::vector<float>& vHeigh)
{
...
bool CWeightedPathNodeMethod::__isOneMoreInBoundBox(const std::vector<float>& vTestPoint, const std::vector<float>& vLow, const std::vector<float>& vHeigh)
{
	int count = 0;
	for (int i = 0; i < vTestPoint.size(); ++i)
	{
...
//****************************************************************************************************
//FUNCTION:
float CWeightedPathNodeMethod::predictWithMinMPOnWholeDimension(const CTree* vTree, const std::vector<float>& vFeatures)
{
	_ASSERTE(vTree);
	std::stack<const CNode*> NodeStack;
	const CNode* TreeRoot = &vTree->getRoot();
	...
	while (!NodeStack.empty())
	{
		const CNode* pCurrentNode = NodeStack.top();
		std::pair<std::vector<float>, std::vector<float>> FeatureRange = pCurrentNode->getFeatureRange();
		bool IsInBox = __isOneMoreInBoundBox(vFeatures, FeatureRange.first, FeatureRange.second);
		if (IsInBox)
		{
			if (pCurrentNode->isLeafNode())
			{
				...
				std::pair<int, float> MinMPAndIndex = pTraingSet->calMinMPAndIndex(pCurrentNode->getNodeDataIndexV(), vFeatures);
		...
		else
		{
			...
			std::vector<int> DataIndex = pCurrentNode->isLeafNode() ? pCurrentNode->getNodeDataIndexV() : __calInterNodeDataIndex(pCurrentNode);
			CTrainingSet *pTraingSet = CTrainingSet::getInstance();
			std::pair<int, float> MinMPAndIndex = pTraingSet->calMinMPAndIndex(DataIndex, vFeatures);
			return pTraingSet->getResponseValueAt(MinMPAndIndex.first);
		}
	}
	//Level.close();
