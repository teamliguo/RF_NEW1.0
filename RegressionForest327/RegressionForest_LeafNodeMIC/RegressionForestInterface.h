...
namespace hiveRegressionForest
{
	...
	//for single response
	REGRESSION_FOREST_EXPORTS float						hivePredict(const std::vector<float>& vTestInstance, unsigned int vForestId, float& voLPPredictSet, float& voMPPredictSet, float& voMPDissimilarity, bool vIsWeightedPrediction = true);
	REGRESSION_FOREST_EXPORTS float 					hivePredict(const std::vector<float>& vTestInstance, unsigned int vForestId, unsigned int vNumOfUsingTrees, float& voLPPredictSet, float& voMPPredictSet, float& voMPDissimilarity, bool vIsWeightedPrediction = true);
