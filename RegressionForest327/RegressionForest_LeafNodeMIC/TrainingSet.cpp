...
float CTrainingSet::calMPDissimilarityGlobal(const std::vector<int>& vLeafIndex, const std::vector<float>& vFeatures, float vPredictResponse)
{
	float SumMP = 0.f;
	std::ofstream MPMIC;
	MPMIC.open("MPMIC.csv", std::ios::app);
	MPMIC << "MP: " << ",";
...
//****************************************************************************************************
//FUNCTION:
double * CTrainingSet::transVector2Array(const std::vector<float>& vData)
{
	double *Test = new double[vData.size()];
	if (!vData.empty())
	{
		memcpy(Test, &vData[0], vData.size()*sizeof(double));
	}
	return Test;
}
...
//****************************************************************************************************
//FUNCTION:
std::vector<float> CTrainingSet::calMIC(const std::vector<float>& vTest, const std::vector<int>& vDataIndex)
{
	std::ofstream MPMIC;
	MPMIC.open("MPMIC.csv", std::ios::app);
	MPMIC << "MIC: " << ",";
	std::vector<float> MIC(vDataIndex.size());
	double* Test = transVector2Array(vTest);
	MINE Mine(0.6, 15, EST_MIC_APPROX);
	for (int i = 0; i < vDataIndex.size(); i++)
	{
		std::vector<float> TrainData = getFeatureInstanceAt(vDataIndex[i]);
		double* Train = transVector2Array(TrainData);
		Mine.compute_score(Test, Train, vTest.size());
		MIC[i] = (float)Mine.mic();
		MPMIC << MIC[i] << ",";
	}
	MPMIC << std::endl;
	MPMIC.close();
