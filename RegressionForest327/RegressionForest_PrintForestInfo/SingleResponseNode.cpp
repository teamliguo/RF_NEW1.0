...
void CSingleResponseNode::outputLeafNodeInfoV(const std::string & vFilePath) const
{
	...
	CTrainingSet* pTrainSet = CTrainingSet::getInstance();
	for (auto iter : LeafNodeDataIndex)
	{
		BootstrapDataset.first.push_back(pTrainSet->getFeatureInstanceAt(iter));
		BootstrapDataset.second.push_back(pTrainSet->getResponseValueAt(iter));
	}
