...
float CTrainingSet::calMPDissimilarityGlobal(const std::vector<int>& vLeafIndex, const std::vector<float>& vFeatures, float vPredictResponse)
{
	float MPParam = 2.0f;
	float SumMP = 0.f;
	float NodeResponseRange = 0.f;
	std::vector<std::vector<float>> LeafNodeFeatureValue;
	std::vector<std::pair<float, int>> OrderStandardDeviation;
	std::vector<float> NodeFeatureRange;
	std::vector<float> LeafNodeResponseValue;
	//std::ofstream DataRange("DataRange.csv");
	for (int i = 0; i < vLeafIndex.size(); i++)
	{
		...
		LeafNodeResponseValue.push_back(getResponseValueAt(vLeafIndex[i]));
		...
		//计算落入两个数据AABB中的数据的y的范围
		//MPValueSum += pow(((float)EachFeatureIntervalCount[0] / (float)getNumOfInstances()) * ((ResponseRange[0].first - ResponseRange[0].second) / (getResponseRange().second - getResponseRange().first)) , MPParam);

		/*std::vector<float> FeatureRangeRate(MaxMinValue.size(), 0.f);
		std::vector<float> ResponseRangeRate(MaxMinValue.size(), 0.f);
		for (int k = 0; k < MaxMinValue.size(); k++)
		{
			FeatureRangeRate[k] = (MaxMinValue[k].first - MaxMinValue[k].second) / (getFeatureRange().second[k] - getFeatureRange().first[k]);
			ResponseRangeRate[0] = (ResponseRange[0].first - ResponseRange[0].second) / (getResponseRange().second - getResponseRange().first);
			//std::cout << FeatureRangeRate[k] << "," << ResponseRangeRate[k]<< std::endl;
			DataRange << FeatureRangeRate[k] << "," << ResponseRangeRate[0] << std::endl;
		}*/
		
		SumMP += pow(MPValueSum, 1 / MPParam);
	}

	/*std::ofstream DataRange;
	DataRange.open("DataRange.csv", std::ios::app);
	NodeResponseRange = *std::max_element(LeafNodeResponseValue.begin(), LeafNodeResponseValue.end()) - *std::min_element(LeafNodeResponseValue.begin(), LeafNodeResponseValue.end());
	for (int i = 0; i < LeafNodeFeatureValue[0].size(); i++)
	{
		std::vector<float> Column(LeafNodeFeatureValue.size());
		for (int k = 0; k < LeafNodeFeatureValue.size(); k++)
			Column[k] = LeafNodeFeatureValue[k][i];
		float max = *std::max_element(Column.begin(), Column.end());
		float min = *std::min_element(Column.begin(), Column.end());
		NodeFeatureRange.push_back(max - min);
	}

	for (int i = 0; i < NodeFeatureRange.size(); ++i)
	{
		DataRange << NodeFeatureRange[i] << ",";
	}
	DataRange << NodeResponseRange << std::endl;
...
void CTrainingSet::__countIntervalNode(const std::vector<std::pair<float, float>>& vMaxMinValue, const std::vector<std::vector<float>>& vFeatureDataSet, std::vector<int>& voIntervalCount, std::vector<std::pair<float, float>>& voInterResponseRange)
{
	...
	/*int count = 0;
	for (int i = 0; i < vFeatureDataSet.size(); i++)
	{
		for (int j = 0; j < vFeatureDataSet[0].size(); j++)
		{
			if (vFeatureDataSet[i][j] >= vMaxMinValue[j].first || vFeatureDataSet[i][j] <= vMaxMinValue[j].second)
			{
				break;
			}
			if (j == vFeatureDataSet[0].size() - 1)
			{
