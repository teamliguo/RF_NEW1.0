...
float CWeightedPathNodeMethod::predictWithMinMPOnWholeDimension(const CTree* vTree, const std::vector<float>& vFeatures)
{
	...
	while (!NodeStack.empty())
	{
		...
		if (IsInBox)
		{
			if (pCurrentNode->isLeafNode())
			{
				std::pair<int, float> MinMPAndIndex = pMpCompute->calMinMPAndIndex(vTree, pCurrentNode->getNodeDataIndexV(), vFeatures);
		...
		else
		{
			...
			std::pair<int, float> MinMPAndIndex = pMpCompute->calMinMPAndIndex(vTree, DataIndex, vFeatures);
			return pTrainingSet->getResponseValueAt(MinMPAndIndex.first);
		}
	}
