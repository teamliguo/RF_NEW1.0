...
//去重复值
std::vector<int> CWeightedPathNodeMethod::uniqueData(std::vector<int>& nodeDataIndex)
{
	std::vector<int> uniqueNodeDataIndex;
	uniqueNodeDataIndex.push_back(nodeDataIndex[0]);
	for (int i = 1;i < nodeDataIndex.size();i++)
	{
		int count = 0;
		for (int j = 0;j < uniqueNodeDataIndex.size();j++)
		{

			if (nodeDataIndex[i] != uniqueNodeDataIndex[j])
			{
				count++;
		...
		if (count == uniqueNodeDataIndex.size())
		{
			uniqueNodeDataIndex.push_back(nodeDataIndex[i]);
		}

	}
	return uniqueNodeDataIndex;
}
//****************************************************************************************************
//FUNCTION:
...
//find the children's nodeindex
std::vector<int> CWeightedPathNodeMethod::calNodeDataIndex(const CNode* vNode)//add-zy
{
	std::vector<int> voNodeDataIndex;
	voNodeDataIndex=__calInterNodeDataIndex(vNode);
	return voNodeDataIndex;
}

//FUNCTION:COMPUTE THE LP DISTANCE ,GET THE MIN MP AND LP SAMPLE'S RESPONSE--BY ZY 2.18
std::pair<float, float> CWeightedPathNodeMethod::predictWithMinMPAndLPOnWholeDimension(const CTree* vTree, const std::vector<float>& vFeatures, std::vector<int>& voNodeDataIndex, int vInstanceMethod,float vInstanceNumber,float vInstanceNumberRation)
{
	_ASSERTE(vTree);
	std::stack<const CNode*> NodeStack;
	const CNode* TreeRoot = &vTree->getRoot();
	...
	while (!NodeStack.empty())
	{
		...
		if (IsInBox)
		{
			if (pCurrentNode->isLeafNode())
			{
				...
				float responeLp = 0.f;
				CTrainingSet *pTraingSet = CTrainingSet::getInstance();
			/*	std::vector<std::vector<float>> LocatedNodeDataFeature(voNodeDataIndex.size(), std::vector<float>());
				std::vector<float> LocatedNodeDataResponse(voNodeDataIndex.size(), 0.f);*/
			/*	for (int i = 0; i < voNodeDataIndex.size(); ++i)
				{
				LocatedNodeDataFeature[i] = pTraingSet->getFeatureInstanceAt(voNodeDataIndex[i]);
				LocatedNodeDataResponse[i] = pTraingSet->getResponseValueAt(voNodeDataIndex[i]);
				}
				std::vector<std::pair<int, float>>  MinMPAndIndex = pTraingSet->calMinMPAndIndex(LocatedNodeDataFeature, LocatedNodeDataResponse,voNodeDataIndex, vFeatures, vInstanceMethod, vInstanceNumber,vInstanceNumberRation);
				std::vector<std::pair<int, float>>  MinLPAndIndex = pTraingSet->calMinLPAndIndex(voNodeDataIndex, vFeatures,vInstanceMethod, vInstanceNumber, vInstanceNumberRation);
				
				for (int i = 0; i < MinMPAndIndex.size(); i++)
				{
				    responeMp += pTraingSet->getResponseValueAt(MinMPAndIndex[i].first);
					 
				}

				for (int i = 0; i < MinLPAndIndex.size(); i++)
				{
				 
					responeLp += pTraingSet->getResponseValueAt(MinLPAndIndex[i].first);
				}

				responeMp = responeMp / MinMPAndIndex.size();
				responeLp = responeLp / MinLPAndIndex.size();*/
				return std::make_pair( responeMp,  responeLp);
			}
			NodeStack.pop();
			if (vFeatures[pCurrentNode->getBestSplitFeatureIndex()] < pCurrentNode->getBestGap())
				NodeStack.push(&pCurrentNode->getLeftChild());
...
//FUNCTION:COMPUTE THE LP DISTANCE ,GET THE MIN MP AND LP  And Similar with IForest Way SAMPLE'S RESPONSE--BY ZY 3.13
std::pair<float, float> CWeightedPathNodeMethod::predictWithMinMPAndLPAndIForestWay(const CTree* vTree, const std::vector<float>& vFeatures, std::vector<int>& voNodeDataIndex, int vInstanceMethod, float vInstanceNumber, float vInstanceNumberRation)
{
	_ASSERTE(vTree);
	std::stack<const CNode*> NodeStack;
	const CNode* TreeRoot = &vTree->getRoot();
