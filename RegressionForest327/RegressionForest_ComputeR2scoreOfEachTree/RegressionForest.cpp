...
//****************************************************************************************************
//FUNCTION:
float CRegressionForest::predict(const std::vector<float>& vFeatures, unsigned int vNumOfUsingTrees, std::vector<std::vector<float>>& voTreePredictSet, std::vector<std::vector<float>>& voTreeDissimilarity, bool vIsWeightedPrediction) const
{ 
	return __predictCertainResponse(vFeatures, vNumOfUsingTrees, vIsWeightedPrediction, voTreePredictSet, voTreeDissimilarity);
}
...
void CRegressionForest::predict(const std::vector<float>& vFeatures, unsigned int vNumOfUsingTrees, bool vIsWeightedPrediction, unsigned int vNumResponse, std::vector<float>& voPredictValue) const
{
	...
	for (int i = 0; i < vNumResponse; ++i)
		voPredictValue[i] = __predictCertainResponse(vFeatures, vNumOfUsingTrees, vIsWeightedPrediction, std::vector<std::vector<float>>(), std::vector<std::vector<float>>(), i);
}

//****************************************************************************************************
//FUNCTION:
float CRegressionForest::__predictCertainResponse(const std::vector<float>& vFeatures, unsigned int vNumOfUsingTrees, bool vIsWeightedPrediction, std::vector<std::vector<float>>& voTreePredictSet, std::vector<std::vector<float>>& voTreeDissimilarity, unsigned int vResponseIndex) const
{
	...
	std::vector<float> TreeDissimilarity(vNumOfUsingTrees, 0.0f);
...
//#pragma omp parallel for
	for (int i = 0; i < vNumOfUsingTrees; ++i)
	{
		...
		voTreePredictSet[i].push_back(PredictValueOfTree[i]);
		float Temp = pTrainingSet->calMPDissimilarityGlobal(LeafNodeSet[i]->getNodeDataIndexV(), vFeatures, PredictValueOfTree[i]);
		voTreeDissimilarity[i].push_back(Temp/*pTrainingSet->calMPDissimilarityGlobal(LeafNodeSet[i]->getNodeDataIndexV(), vFeatures, PredictValueOfTree[i])*/);
		TreeDissimilarity[i] = 1.0 / Temp;
	}
	//std::cout << std::endl;.
	...
	if (vIsWeightedPrediction)
	{
		...
		float SumWeight = std::accumulate(NodeWeight.begin(), NodeWeight.end(), 0.f);
	...
	{
		float SumWeight = std::accumulate(TreeDissimilarity.begin(), TreeDissimilarity.end(), 0.f);
		if (fabs(SumWeight - 0.f) > 1e-6)
		{
			for (int k = 0; k < vNumOfUsingTrees; ++k)
				PredictValue += PredictValueOfTree[k] * TreeDissimilarity[k];
			return PredictValue / SumWeight;
		}
		PredictValue = std::accumulate(PredictValueOfTree.begin(), PredictValueOfTree.end(), 0.f);
