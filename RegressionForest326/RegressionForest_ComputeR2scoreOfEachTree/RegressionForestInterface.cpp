...
//****************************************************************************************************
//FUNCTION:
float hiveRegressionForest::hivePredict(const std::vector<float>& vTestInstance, unsigned int vForestId, std::vector<std::vector<float>>& voTreePredictSet, std::vector<std::vector<float>>& voTreeDissimilarity, bool vIsWeightedPrediction)
{
	...
	return hivePredict(vTestInstance, vForestId, pRegressionForest->getNumOfTrees(), voTreePredictSet, voTreeDissimilarity, vIsWeightedPrediction);
}

//****************************************************************************************************
//FUNCTION:
float hiveRegressionForest::hivePredict(const std::vector<float>& vTestInstance, unsigned int vForestId, unsigned int vNumOfUsingTrees, std::vector<std::vector<float>>& voTreePredictSet, std::vector<std::vector<float>>& voTreeDissimilarity, bool vIsWeightedPrediction)
{
	_ASSERT(!vTestInstance.empty());
	_ASSERT(vNumOfUsingTrees == voTreePredictSet.size());

	const CRegressionForest* pRegressionForest = CRegressionForestPool::getInstance()->fetchForest(vForestId);
	_ASSERTE(pRegressionForest);

	return pRegressionForest->predict(vTestInstance, vNumOfUsingTrees, voTreePredictSet, voTreeDissimilarity, vIsWeightedPrediction);
}
