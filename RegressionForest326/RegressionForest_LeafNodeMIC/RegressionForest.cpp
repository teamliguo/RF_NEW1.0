...
float CRegressionForest::__predictCertainResponse(const std::vector<float>& vFeatures, unsigned int vNumOfUsingTrees, bool vIsWeightedPrediction, float& voLPPredictSet, float& voMPPredictSet, float& voMPDissimilarity, unsigned int vResponseIndex) const
{
	...
	std::vector<float> EuclideanDistanceWeight(vNumOfUsingTrees, 0.0f);
...
//#pragma omp parallel for
	for (int i = 0; i < vNumOfUsingTrees; ++i)
	{
		...
		//MPDistanceWeight[i] = pTrainingSet->calMPDissimilarityGlobal(LeafNodeSet[i]->getNodeDataIndexV(), vFeatures, PredictValueOfTree[i]);
		//EuclideanDistanceWeight[i] = pTrainingSet->calEuclideanDistance(LeafNodeSet[i]->getNodeDataIndexV(), vFeatures);
		std::vector<float> TrainDataMIC = pTrainingSet->calMIC(vFeatures, LeafNodeSet[i]->getNodeDataIndexV());
	}
	//std::cout << std::endl;

	float MinEuclideanWeight = *std::min_element(EuclideanDistanceWeight.begin(), EuclideanDistanceWeight.end());
	float MaxEuclideanWeight = *std::max_element(EuclideanDistanceWeight.begin(), EuclideanDistanceWeight.end());
	if (MaxEuclideanWeight > MinEuclideanWeight)
		for (int i = 0; i < EuclideanDistanceWeight.size(); ++i)
			EuclideanDistanceWeight[i] = 1 - (EuclideanDistanceWeight[i] - MinEuclideanWeight) / (MaxEuclideanWeight - MinEuclideanWeight);
	else
		EuclideanDistanceWeight = std::vector<float>(vNumOfUsingTrees, 1.0f);

	float MinMPWeight = *std::min_element(MPDistanceWeight.begin(), MPDistanceWeight.end());
	float MaxMPWeight = *std::max_element(MPDistanceWeight.begin(), MPDistanceWeight.end());
	if (MaxMPWeight > MinMPWeight)
		for (int i = 0; i < MPDistanceWeight.size(); ++i)
			MPDistanceWeight[i] = 1 - (MPDistanceWeight[i] - MinMPWeight) / (MaxMPWeight - MinMPWeight);
	else
		MPDistanceWeight = std::vector<float>(vNumOfUsingTrees, 1.0f);

	voMPDissimilarity = std::accumulate(MPDistanceWeight.begin(), MPDistanceWeight.end(), 0.f) / vNumOfUsingTrees;
	...
	else
	{
		for (int k = 0; k < vNumOfUsingTrees; ++k)
		{
			voLPPredictSet += PredictValueOfTree[k] * EuclideanDistanceWeight[k];
			//voMPPredictSet += PredictValueOfTree[k] * MPDistanceWeight[k];
		}
		float SumLPWeight = std::accumulate(EuclideanDistanceWeight.begin(), EuclideanDistanceWeight.end(), 0.0f);
		//float SumMPWeight = std::accumulate(MPDistanceWeight.begin(), MPDistanceWeight.end(), 0.0f);
