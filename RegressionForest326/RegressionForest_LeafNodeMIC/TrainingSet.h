...
namespace hiveRegressionForest
{
	...
	public:
		...
		std::pair<int, float>       calMinMPAndIndex(const std::vector<int>& vDataIndex, const std::vector<float>& vFeature);//add-gss-1.16
		double*                     transVector2Array(const std::vector<float>& vData);//add-gss-19.3.18
		std::vector<float>          calMIC(const std::vector<float>& vTest, const std::vector<int>& vDataIndex);//add-gss-19.3.18
