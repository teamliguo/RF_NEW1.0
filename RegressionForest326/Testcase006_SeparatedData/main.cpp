...
#include "../RegressionForest_SeparatedData/RegressionForestInterface.h"
#include "../RegressionForest_SeparatedData/RegressionForestCommon.h"
#include "../RegressionForest_SeparatedData/TrainingSet.h"
#include "../RegressionForest_SeparatedData/TrainingSetConfig.h"
#include "../RegressionForest_SeparatedData/TrainingSetCommon.h"
#include "../RegressionForest_SeparatedData/Utility.h"
...
void main()
{
	...
	try
	{
		...
		{
			std::cout << "Predicting£º" << Index << "/" << TestResponseSet.size() << std::endl;
			PredictSet[Index] = hiveRegressionForest::hivePredict(TestFeatureSet[Index], ForestId, MPPredictSet[Index], MPDissimilarity[Index], false);
			float Bias = std::abs(MPPredictSet[Index] - TestResponseSet[Index]);
			PredictInfo << Bias / TestResponseSet[Index] << std::endl;
		}
		PredictInfo.close();
		...
		std::ofstream BadData, GoodData;
		BadData.open("BadData.csv", std::ios::app);
		GoodData.open("GoodData.csv", std::ios::app);
		double MeanBias = std::accumulate(BiasSet.begin(), BiasSet.end(), 0.f) / BiasSet.size();
		for (int i = 0; i < TestFeatureSet.size(); ++i)
		{
			if (BiasSet[i] > MeanBias)
			{
				for (int k = 0; k < TestFeatureSet[i].size(); k++)
				{
					BadData << TestFeatureSet[i][k] << ",";
				}
				BadData << TestResponseSet[i] << std::endl;
			}
			else
			{
				for (int k = 0; k < TestFeatureSet[i].size(); k++)
				{
					GoodData << TestFeatureSet[i][k] << ",";
				}
				GoodData << TestResponseSet[i] << std::endl;
			}
		}
		BadData.close();
		GoodData.close();
		...
		std::cout << "Inter MP MSE: " << MPMSE << std::endl;
		std::cout << "Inter MP R2Score: " << MPR2Score << std::endl;
		...
		StatisticalResultFile << "Inter MP MSE: " << MPMSE << std::endl;
		StatisticalResultFile << "Inter MP R2Score: " << MPR2Score << std::endl;
