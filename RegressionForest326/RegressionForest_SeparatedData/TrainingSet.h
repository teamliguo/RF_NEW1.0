...
namespace hiveRegressionForest
{
	...
	public:
		...
		static bool compareTuple(const std::tuple<std::vector<float>, float, float>& vFirst, std::tuple<std::vector<float>, float, float>& vSecond);//Add_ljy_12/2
		...
		std::vector<std::tuple<std::vector<float>, float, float>> calReCombineDataMP(const std::vector<std::vector<float>>& vAllLeafFeatureSet, const std::vector<float>& vAllLeafResponseSet, const std::vector<float>& vFeatures, float vPredictResponse);
